  ...
  return (
    <div className="rounded-2xl border border-slate-200 dark:border-[#2a2a2a] bg-white dark:bg-[#141414] p-4">
      <div className="mb-3 flex items-center gap-2">
        ...
        <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-100">GPU par nœud</h3>
      </div>
      {loading ? (
        <p className="text-[12px] text-slate-500 dark:text-slate-400 dark:text-slate-500">Chargement…</p>
      ) : gpus.length === 0 ? (
        <p className="text-[12px] text-slate-500 dark:text-slate-400 dark:text-slate-500">Aucune donnée GPU disponible.</p>
      ) : (
            ...
            return (
              <div key={`${g.node}-${i}`} className={`rounded-xl border bg-gradient-to-b to-transparent p-3 ${hostAccent(g.node)}`}>
                <div className="flex items-center justify-between">
                  ...
                  {g.available && (
                    <span className="flex items-center gap-1 text-[11px] text-slate-600 dark:text-slate-400 dark:text-slate-500">
                      <Thermometer size={12} /> {g.temp_c}°C
                    </span>
                  )}
                </div>
                {!g.available ? (
                  <p className="mt-2 text-[12px] text-slate-500 dark:text-slate-400 dark:text-slate-500">GPU indisponible</p>
                ) : (
                  <>
                    <p className="mt-1 truncate text-[12px] text-slate-600 dark:text-slate-400 dark:text-slate-500">{g.gpu}</p>
