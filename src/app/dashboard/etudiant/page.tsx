...
export default function StudentDashboard() {
  ...
  const navItems = [
    ...
    { id: 'requests' as const, label: 'Mes Requêtes', icon: FileText, badge: pendingRequests },
    { id: 'publications' as const, label: 'Mes Publications', icon: BookOpen },
  ...
  return (
    <div className="relative min-h-screen bg-slate-50 text-slate-900 font-sans pb-24 overflow-x-hidden">
      ...
      {/* ── TOAST ── */}
      {toast && (
        <div className={`fixed bottom-8 right-8 z-50 px-6 py-4 rounded-xl border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex items-center gap-3 transition-all duration-300 animate-slide-up ${toast.type === 'success' ? 'bg-emerald-500 text-white'
          : toast.type === 'danger' ? 'bg-red-500 text-white'
            : 'bg-blue-600 text-white'
          }`}>
          <span className="font-black text-sm uppercase">
            {toast.type === 'success' ? '✓ Succès' : toast.type === 'danger' ? '⚠ Alerte' : 'ℹ Info'} :
          </span>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-[8px] font-black px-1.5 py-0.5 rounded bg-emerald-50 text-emerald-600 border border-emerald-200 uppercase">ESPACE ÉTUDIANT</span>
            </div>
            <p className="text-[10px] font-bold tracking-widest text-slate-500 uppercase mt-0.5">
              PORTAIL ACADÉMIQUE · ENSPY · GÉNIE INFORMATIQUE
            </p>
          </div>
        </div>
