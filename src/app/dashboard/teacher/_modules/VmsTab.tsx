...
interface VmsTabProps {
  requests: VmRequest[];
  onApprove: (id: string, studentName: string, projectName: string) => void;
  onReject: (id: string, studentName: string) => void;
}
...
export default function VmsTab({ requests, onApprove, onReject }: VmsTabProps) {
  ...
  const approve = (req: VmRequest) => { onApprove(req.id, req.studentName, req.projectName); setSelected(null); };
  const reject  = (req: VmRequest) => { onReject(req.id, req.studentName); setSelected(null); };
  ...
  return (
    <>
      <div className="relative bg-white border border-slate-200 rounded-2xl p-8 pb-14 space-y-6 shadow-sm hover:shadow-md transition-shadow duration-300">
        ...
        <Screw3D className="bottom-2 left-2 rotate-45"   />
        <Screw3D className="bottom-2 right-2 -rotate-[45deg]"/>
              ...
              >
                <div className="space-y-2">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="text-base font-black text-black">Projet : "{req.projectName}"</span>
                  </div>
                  <div className="flex flex-wrap items-center gap-y-1 gap-x-4 text-xs font-semibold text-slate-500">
                    <p><span className="text-slate-400">Étudiant :</span> {req.studentName} ({req.matricule})</p>
                    <p className="flex items-center gap-1"><Cpu className="w-3.5 h-3.5 text-slate-400" />{req.vcpu} vCPUs</p>
                    <p className="flex items-center gap-1"><Layers className="w-3.5 h-3.5 text-slate-400" />{req.ram} Go RAM</p>
                    <p className="flex items-center gap-1"><HardDrive className="w-3.5 h-3.5 text-slate-400" />{req.storage} Go HDD</p>
                  </div>
                </div>
          ...
          <div className="relative bg-white border border-slate-200 rounded-2xl w-full max-w-lg p-8 overflow-hidden shadow-xl">
            ...
            <div className="space-y-4 text-xs font-semibold text-slate-700 mb-8">
              <div className="flex justify-between border-b border-slate-100 pb-3">
                <p><span className="text-slate-400 font-bold uppercase text-[9px]">Demandeur</span> {selected.studentName} ({selected.matricule})</p>
                <p className="text-right"><span className="text-slate-400 font-bold uppercase text-[9px]">Date</span> {selected.date}</p>
              </div>

              <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl grid grid-cols-2 gap-4">
                <p className="flex items-center gap-2"><Cpu className="w-4 h-4 text-blue-600 shrink-0" /> <span className="font-bold">{selected.vcpu} vCPUs</span></p>
                <p className="flex items-center gap-2"><Layers className="w-4 h-4 text-blue-600 shrink-0" /> <span className="font-bold">{selected.ram} Go RAM</span></p>
                <p className="flex items-center gap-2"><HardDrive className="w-4 h-4 text-blue-600 shrink-0" /> <span className="font-bold">{selected.storage} Go SSD</span></p>
                <p className="flex items-center gap-2"><FileCode2 className="w-4 h-4 text-blue-600 shrink-0" /> <span className="font-bold">{selected.os}</span></p>
              </div>

              <div className="space-y-3">
                <p><span className="text-slate-400 font-bold block uppercase text-[9px] mb-0.5">Réseau (VLAN ID)</span>{selected.networkVlan}</p>
                <div className="p-4 bg-blue-50/50 border border-blue-200 rounded-xl space-y-1">
                  <span className="text-blue-600 font-bold block uppercase text-[9px]">Usage &amp; Projet associé</span>
                  <p className="leading-relaxed font-medium text-slate-700">Projet : "{selected.projectName}"</p>
                  <p className="leading-relaxed text-slate-600">{selected.purpose}</p>
                </div>
              </div>
            </div>
